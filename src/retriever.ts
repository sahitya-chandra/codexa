...
export async function retrieveContext(
  ...
  const mentionedMarkdownFiles = mentionedFiles.filter(f => 
    f.toLowerCase().endsWith('.md') || f.toLowerCase().includes('readme')
  );
  ...
  const mentionedMarkdownResults = mentionedMarkdownFiles.length > 0
    ? allResults.filter(r => {
        ...
        return mentionedFiles.some(mentioned => 
          matchesFilePattern(r.filePath, mentioned)
        );
      })
    : [];
...
export function formatContext(results: RetrievalResult[]): string {
  ...
  results.forEach(r => {
    const isDoc = r.filePath.toLowerCase().endsWith('.md') || 
